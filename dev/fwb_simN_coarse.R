...
# ------------------------------- SETTINGS -----------------------------------
n_cores   <- 8L
N_grid <- c(50L, 80L, 150L, 300L, 600L)
M_truth <- 500L                 # datasets for the true SD + asymptotic coverage
m_fwb <- 60L                  # subset of those also given an FWB run
R <- 250L                  # FWB weight draws per dataset
fwb_types <- c("exp", "beta", "power")
mu_off    <- 1.0                   # off-target latent-mean shift (logits)
conf      <- 0.95
seed0     <- 2024L
out_file  <- "dev/fwb_simN_coarse.rds"
# ----------------------------------------------------------------------------
