...
asked for when total scores tie at the boundaries, equal-width and
manual grouping can define intervals no respondent falls into, and both
binned methods fall back to score level on a degenerate score
distribution. The caption reports the realised number of groups, how
many of them contain respondents when that is fewer, and any fall back.
