...
test_that("RMitemICCPlot errors when dif_var length mismatches nrow(data)", {
  ...
  expect_error(RMitemICCPlot(df, dif_var = grp),
               regexp = "same length")
})
...
test_that("RMitemICCPlot output = 'list' returns one ggplot per item, named after the items", {
  skip_if_not_installed("iarm")
  skip_if_not_installed("ggplot2")
  df <- make_polytomous()
...
test_that("RMitemICCPlot DIF mode returns a composite", {
  skip_if_not_installed("iarm")
  skip_if_not_installed("patchwork")
  skip_if_not_installed("ggplot2")
...
test_that("RMitemICCPlot method = 'score' returns a composite", {
  skip_if_not_installed("iarm")
  skip_if_not_installed("patchwork")
  skip_if_not_installed("ggplot2")
  df <- make_polytomous()
  p <- RMitemICCPlot(df, method = "score")
  expect_s3_class(p, "patchwork")
})
...
test_that("RMitemICCPlot drops rows with NA in data or dif_var", {
  ...
  expect_s3_class(
    RMitemICCPlot(df, class_intervals = 4L, dif_var = grp),
    "patchwork"
  )
})
