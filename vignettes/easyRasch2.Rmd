...
You can also get a plot summarizing simulated and observed item infit, using `RMinfitcutoffPlot()`. Since conditional infit needs complete data, there is a sibling function that uses multiple imputation with infit that is useful if you have partial missingness in your data - `RMiteminfit_mi()` and `RMinfitcutoff_mi()`.
...
You can also get simulation-based thresholds for partial gamma LD, using `RMpgLDcutoff()`, which can be used with `RMpartgamLD()` and also to plot the results with `RMpgLDplot()`
...
|RMU (WLE)        |    0.883|            0.867|            0.898|200 PVs, 20 RMU iterations |
